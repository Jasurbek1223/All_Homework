...
//var customfilter = new CustomerFilterModel
//{
//    Count = 15,
//    Amount = 3000
//};

//customers.Add(new Customer(100, 18, 3900, new DateTime(2023, 9, 10)));

//var query = customers.Where(customers => customers.Count >= customfilter.Count && customfilter.Amount >= customfilter.Amount);

//foreach (var customer in query)
//{
//    Console.WriteLine($"Id: {customer.Id}, Count: {customer.Count}, Amount: {customer.Amount}");
//}
...
if (filter.Count > 0)
{
    query = query.Where(c => c.Count >= filter.Count);
}

if (filter.Amount > 0)
{
    query = query.Where(c => c.Amount >= filter.Amount);
}

if (filter.Date != default)
{
    query = query.Where(c => c.Date >= filter.Date);
}

Console.WriteLine("Filtered Customers:");
foreach (var customer in query)
{
    Console.WriteLine($"ID: {customer.Id}, Count: {customer.Count}, Amount: {customer.Amount}, Date: {customer.Date}");
}

// Yana bitta customer qo'shish
//var newCustomer = new Customer { Count = 2, Amount = 180, Date = new DateTime(2023, 4, 20) };
//customers.Add(newCustomer);

Console.WriteLine("\nCustomers after adding a new customer:");
foreach (var customer in query)
{
    Console.WriteLine($"ID: {customer.Id}, Count: {customer.Count}, Amount: {customer.Amount}, Date: {customer.Date}");
}
